...
function createPostPushPutOutput(verbs, definitions, pathKeys) {
	...
	if (verbs && verbs.success && verbs.success.fields && verbs.success.fields["Success 200"]) {
		pathItemObject[verbs.type].responses = createResponses(verbs.success.fields["Success 200"]);
	}
...
function createResponses(fields) {
	var schema = {};
	var properties = {};
	if (fields.length === 1) {
		var field = fields[0];
		...
		if (type === "array") {
			schema.items = {
				type: field.type.slice(0, -2).toLowerCase()
			}
		}
	} else {
		var properties = {};
